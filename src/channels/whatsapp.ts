...
import { isVoiceMessage, transcribeAudioFile } from '../modules/voice-transcription/index.js';
import { markInboundVoice, shouldMirrorVoice, synthesizeSpeech } from '../modules/tts-mirror/index.js';
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // MODULE-HOOK:voice-transcription — replace audio attachments
            // that are WhatsApp voice notes (ptt=true or opus codec) with an
            // inline `[Voice: <transcript>]` prefix on the message text.
            const incomingIsVoice = !!(normalized.audioMessage && isVoiceMessage(normalized.audioMessage));
            if (incomingIsVoice) {
            ...
            // MODULE-HOOK:tts-mirror — remember whether the most recent
            // inbound on this chat was a voice note. The deliver() hook
            // checks this to decide whether to ALSO send a TTS audio reply.
            markInboundVoice(chatJid, incomingIsVoice);
    ...
    const adapter: ChannelAdapter = {
      ...
      async deliver(
        ...
        if (text) {
          ...
          const msgId = await sendRawMessage(platformId, prefixed);

          // MODULE-HOOK:tts-mirror — if the user's most recent inbound was
          // a voice note, also send a TTS-synthesised voice note alongside
          // the text. Best-effort: failures are logged and ignored since
          // the text already went out.
          if (shouldMirrorVoice(platformId)) {
            try {
              ...
              log.warn('Voice mirror send failed (text already sent)', {
                platformId,
                err,
              });
            }
